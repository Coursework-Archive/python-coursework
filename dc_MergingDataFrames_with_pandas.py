...
#using a loop to import csv files to convert them into DataFrames
...
#using a list comprehension to import csv files
filenames = ['sales-jan-2015.csv', 'sales-feb-2015.csv']

dataframes = [pd.read_csv(f) for f in filenames]

#using glob, when filenames have the same pattern
...
w_mean2 = w-mean.reindex(ordered)
print(w_mean2)

#original DataFrame can be recovered using .sort_index()
w_mean2.sort_index()
...
w_max.reindex(w_mean.index).dropna()

##Sortnig DataFrame with the Index & columns
# Import pandas
import pandas as pd
...
#slice the min and the max as a DataFrame week1_range
week1_range = weather.loc['2013-07-01':'2013-07-07','Min TemperatureF', 'Max TemperatureF']
print(week1_range)
#slice the mean temperature as a Series week1_mean
week1_mean = weather.loc['2013-07-01':'2013-07-07','Mean TemperatureF']
print(week1_mean)

#dividing the series week1_range / week1_mean doesn't work; the column label don't match so the result has
#all null values
#Instead using .divide will allow more ways to control how the arithmatic is performed
week1_range.divide(week1_mean, axis='rows')

#Can also compute a ercentage change along a data Series (previous day value - current day value / previous day value)
...
##Appending and concatenating Sales
#.apend(): Series and DataFrame method
#Invocation: s1.append(s2) stack rows of s2 below s1
...
##Appending DataFraes with ignore_index
...
##Concatenating vertically to get Multindexed rows
for medal in medal_types:
    file_name = "%s_top5.csv" % medal
...
#Slicing Multindexed DataFrames
...
#Concatenating horizonatally to get MultiIndexed columns
...
#Concatenating DataFrames from dict
...
#Outer join - missing fields filled with NaN
...
#Resample & concatening DataFrames with inner join
...
#Allowing matching on multiple columns
pd.merge(bronze, gold, on=['NOC', 'Country'])

#Columns can be renamed to match
...
#Using .join(how='left')
#joins columns and aligns the text on the left and uses the index of the
#left DataFrame; and the opposite is true for the how='right'

#df1.append(df2): stacking vertically
#pd.concat([df1, df2]):
##stacking many horizontally or vertically
##simple inner/outer joins on Indexs
#df1.join(df2) inner/out/left/right joins on indexes
#pd.merge([df1, df2])
...
#Loading Olympic edition DataFram
